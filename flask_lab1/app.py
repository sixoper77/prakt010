...
def catalog():
    ...
    return render_template('catalog.html', products=filtered, categories=categories,
                           current_category=category, search_query=None)
...
def search():
    ...
    categories = sorted(set(p['category'] for p in products))
    return render_template('catalog.html', products=results, categories=categories,
                           current_category=None, search_query=query)
...
def api_products():
    ...
    result = apply_filters(products, category=category, min_price=min_price, max_price=max_price)
